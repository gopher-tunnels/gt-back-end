...
interface RouteResult {
  steps: (RouteStep | StepInfo)[]; // In order of the path.
...
/**
 * Helper function that extracts step information from the response
 *
 * @param steps - A list of steps generated by the Google Maps API
 * @returns A list of steps parsed down to our custom type StepInfo with type 'GM'
 */
function extractStepInfoFromResponse(steps: any[]): StepInfo[] {
    ...
    try {
      const pathToClosestBuildResponse = await axios.get(googleMapsUrl, {
        params: {
          ...
          destination: `${startNode.latitude}, ${startNode.longitude}`,
          mode: "walking",
          key: googleMapsKey
        },
      });
      ...
      console.error("Google Maps Directions API failed:", err);
      // Proceed with Neo4j-only route, no GM steps
    }
